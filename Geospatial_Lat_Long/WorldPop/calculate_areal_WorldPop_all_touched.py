# Calculation for population age sex will be wrong!
...
def insert_data_to_db(data, conn, chunk_size=100000):
    ...
    for i in range(0, len(data), chunk_size):
        ...
        query = """
        INSERT INTO geospatial_data_worldpop (gid, admin_level, date, variable, sum, mean, min, max, missing_value_percentage, source, unit)
...
def calculate_daily_stats(da, geometry, cell_area, buffer_size=2):
    ...
    :return: Tuple of daily sum, mean, min, max, and missing value percentage
    """
    try:
...
def process_geometry(da, variable_name, geometry, level, gid, cell_area, unit):
    """
    Process a single geometry to calculate daily statistics and prepare data for database insertion.

    :param da: xarray DataArray with daily data
    ...
    """
    daily_sum, daily_mean, daily_min, daily_max, missing_value_percentage = (
        calculate_daily_stats(da, geometry, cell_area)
    )
    ...
    for date in da.time.values:
        ...
        if daily_sum is None:
            ...
            sum_val = daily_sum.sel(time=date).values
...
def process_batch(batch, da, var_name, level, cell_area, unit):
    """
...
def find_files(data_directory, variable_name):
    ...
    """
    file_pattern = (
        f"Global_Population_{variable_name.split('_')[-1].capitalize()}*.nc"
    )

    if variable_name == "population_age_sex_count":
        file_pattern = "global_*.nc"
...
def process_level(
    ...
    :param data_directory: Directory containing netCDF files
    :param variables: Dictionary of variables to process
    ...
    for var_code, var_name in variables.items():
        with Pool(processes=6) as pool:
            ...
            for file_path in matching_files:
                ...
                if var_name == "population_age_sex_count":
                    var_name = (
                        var_name
                        + "_"
                        + "_".join(
                ...
                # Open the dataset outside the loop
                if use_dask:
                ...
                cell_area = calculate_cell_area(da)
                ...
                try:
                    ...
                    total_features = len(
                        gpd.read_file(geopackage_path, layer=f"ADM_{level}")
                    )
                    print(
                        f"total_areal_features (ADM_{level}): {total_features}"
                    )
                    # Process in batches
                    for start_index in range(0, total_features, chunk_size):
                        gdf = gpd.read_file(
                            geopackage_path,
                            layer=f"ADM_{level}",
                            rows=slice(start_index, start_index + chunk_size),
                        )
                        ...
                        with tqdm(
                            total=len(gdf),
                            desc=f"Batch progress: {var_name} - level {level}",
                        ) as batch_pbar:
                            results = []
                            for batch_result in pool.imap(
                                ...
                                batch_pbar.update(len(batch_result))
...
def main():
    ...
    variables = {
        ...
        "population_age_sex_count": "population_age_sex_count",
    }
