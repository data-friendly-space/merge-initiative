...
ALTER TABLE geospatial_data_landcover
ADD CONSTRAINT unique_landcover_entry UNIQUE (gid, admin_level, date, variable);
"""
...
def create_landcover_table():
    """
    Create the LandCover data table in the local PostgreSQL database using the predefined SQL.
    ...
    try:
        ...
        print(
            "LandCover data table created successfully with unique constraint."
        )

    except (Exception, psycopg2.Error) as error:
        print(
            f"Error creating LandCover data table or adding constraint: {error}"
        )
