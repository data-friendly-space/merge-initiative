...
CREATE TABLE IF NOT EXISTS geospatial_data_gleam (
...
ALTER TABLE geospatial_data_gleam
ADD CONSTRAINT unique_gleam_entry UNIQUE (gid, admin_level, date, variable);
"""
...
def create_gleam_table():
    """
    Create the GLEAM data table in the local PostgreSQL database using the predefined SQL.
    ...
    try:
        ...
        print("GLEAM data table created successfully with unique constraint.")

    except (Exception, psycopg2.Error) as error:
        print(f"Error creating GLEAM data table or adding constraint: {error}")
